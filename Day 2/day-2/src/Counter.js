...
function Counter() {
    ...
    const increment = () => { setCount(++count); notiMessage(cCounter) }
    const decrement = () => { setCount(--count); notiMessage(cCounter) }
    ...
    const Checkpoints = () => {
        return (
            <>{
                checkpoints.map((c, i) => <li key={i}>{i + 1}. {c}</li>)
            }</>
        )
    }
    const pauseTimer = () => {
    ...
    const notiMessage = (data) => {
        ...
        setTimeout(() => setOptions({ opacity: '0', data: data }), 5000);
    }
    return (
        <div className='container'>
            ...
            <div className='time'>
                ...
                <div className="changeState">
                    {timerState === "initial" &&
                        <button onClick={startTimer} disabled={timerState !== "initial"}>Start</button>
                    }{timerState === "paused" &&
                        <button onClick={startTimer} disabled={timerState !== "paused"}>Continue</button>
                    }{timerState === "running" &&
                        <button onClick={pauseTimer} disabled={timerState !== "running"}>Pause</button>
                    }{timerState !== "initial" &&
                        <button onClick={resetTimer} disabled={timerState === "initial"}>Stop</button>
                    }
                </div>
                <Message />
                <ul style={{
